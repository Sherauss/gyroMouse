...
The resulting estimation problem is nonlinear because the device orientation evolves on the 3-D rotation manifold and because the acceleration measured at each IMU depends on cross-products of angular velocity and angular acceleration. An Extended Kalman Filter (EKF) could be used for this task, but it requires explicit Jacobian derivations. Instead, we used an Unscented Kalman Filter (UKF), which represents the state distribution with deterministic sigma points and propagates them through the nonlinear process and measurement models \cite{kalman1960,julier2004,wan2000}.
...
For the offline analysis, spatial orientation is represented using a unit quaternion $\mathbf{q}$ to avoid gimbal lock \cite{kuipers1999,crassidis2003}. The full-state UKF tracks the kinematics at the controller's center of mass (COM):
\begin{equation}
\mathbf{x}_k =
\begin{bmatrix}
\mathbf{q}_k^T &
...
The hardware platform consists of an ESP32 microcontroller and three MPU6500 IMUs placed at the center, left, and right positions of the controller body \cite{invensense2014}.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{fig_sensor_fusion_comparison.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{fig_offline_ukf_angular_velocity.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{fig_offline_ukf_angular_acceleration.png}}
...
The Normalized Innovation Squared (NIS) was used to evaluate whether the measurement residuals are statistically consistent with the predicted innovation covariance \cite{barshalom2001}:
\begin{equation}
\epsilon_k =
\boldsymbol{\nu}_k^T
S_k^{-1}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{fig_ukf_consistency_nis.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{fig_live_ukf_lite_angular_velocity.png}}
...
The authors thank Prof. Dr. Engin Masazade for his guidance in EE4084 and EEE7023. During the preparation of this work, the authors utilized ChatGPT to generate structural drafts and improve the wording of specific sections, including the Abstract, Results, Discussion, and Conclusion sections. Following the use of this tool, the authors thoroughly reviewed and edited the material, and assume full responsibility for the final content of the publication.

\begin{thebibliography}{00}
\bibitem{kalman1960}
R. E. Kalman, ``A new approach to linear filtering and prediction problems,'' \textit{Journal of Basic Engineering}, vol. 82, no. 1, pp. 35--45, Mar. 1960.

\bibitem{julier2004}
S. J. Julier and J. K. Uhlmann, ``Unscented filtering and nonlinear estimation,'' \textit{Proceedings of the IEEE}, vol. 92, no. 3, pp. 401--422, Mar. 2004.

\bibitem{wan2000}
E. A. Wan and R. van der Merwe, ``The unscented Kalman filter for nonlinear estimation,'' in \textit{Proc. IEEE Adaptive Systems for Signal Processing, Communications, and Control Symposium}, Lake Louise, AB, Canada, 2000, pp. 153--158, doi: 10.1109/ASSPCC.2000.882463.

\bibitem{kuipers1999}
J. B. Kuipers, \textit{Quaternions and Rotation Sequences}. Princeton, NJ, USA: Princeton University Press, 1999.

\bibitem{crassidis2003}
J. L. Crassidis and F. L. Markley, ``Unscented filtering for spacecraft attitude estimation,'' \textit{Journal of Guidance, Control, and Dynamics}, vol. 26, no. 4, pp. 536--542, Jul.--Aug. 2003.

\bibitem{barshalom2001}
Y. Bar-Shalom, X. R. Li, and T. Kirubarajan, \textit{Estimation with Applications to Tracking and Navigation}. New York, NY, USA: Wiley, 2001.

\bibitem{invensense2014}
InvenSense Inc., ``MPU-6500 product specification,'' Rev. 1.1, Jan. 2014.
\end{thebibliography}
